...
local AtlasLib = {
	["Theme"] = {
	        ["Font"] = "RobotoMono",
		["AccentColor"] = Color3.fromRGB(26, 26, 26),
...
local function AddConn(conn)
    ...
    table.insert(CreateModule.conns, conn)
end
...
function AtlasLib.Main(Name,X,Y)
    ...
    local TabsButtons = CreateModule.Instance("Frame",{
        ...
        BackgroundTransparency = 1;
        BorderSizePixel = 0;
        Position = UDim2.new((1 - 0.82) / 2, 0, 0, 0);
    ...
    local hideConn = InputService.InputBegan:Connect(function(input,IsTyping)
	...
	function InMain.Tab(Text)
        ...
        function InPage.Section(Text)
            ...
            function InSection.KeyBind(Text,func,defkey)
                ...
                local keybindConn = InputService.InputBegan:Connect(function(input)
            ...
            function InSection.TextBox(Text,func,defvalue)
                ...
                TextBox.Focused:Connect(function()
                    TweenService:Create(TextBox, TweenInfo.new(0.3), {
                        BackgroundColor3 = Color3.fromRGB(32,32,37)
                    }):Play()
                    TweenService:Create(Stroke, TweenInfo.new(0.3), {
                        Color = AtlasLib["Theme"]["AccentColor"]
                    }):Play()
            ...
            function InSection.Slider(Text,min,max,func,precise,defvalue)
                ...
                local inputChangedConn = game:GetService("UserInputService").InputChanged:Connect(function(Pressed)
        ...
        local function _cleanup()
            for _, conn in ipairs(CreateModule.conns) do
                pcall(function() conn:Disconnect() end)
            end
            CreateModule.conns = {}

            pcall(function()
                if AtlasLib.ScreenGui and AtlasLib.ScreenGui.Parent then
                    AtlasLib.ScreenGui:Destroy()
                end
            end)

            for _, inst in ipairs(CreateModule.reg) do
                pcall(function()
